...
def process_config(config_path: Path) -> None:
    ...
    labels_mrf = _relabel(result.labels, dataset.y)
    ari = adjusted_rand_score(dataset.y, labels_mrf)

    model = result.model
    viol_before = model["violations_before"]
    viol_after = model["violations_after"]
    n_changed = model["n_changed"]
    n_iters = model["n_iters"]

    # Identify error cells (aligned labels vs ground truth)
    ...
    with PdfPages(out_path) as pdf:
        ...
        mrf_title = (
            ...
            f"violations {viol_before} -> {viol_after}   "
            f"changed={n_changed}   iters={n_iters}"
        )
        fig_mrf = _plot_grid(dataset, labels_mrf, title=mrf_title, error_cells=error_cells)
        pdf.savefig(fig_mrf, bbox_inches="tight"); plt.close(fig_mrf)

        # Page 3: diagnostic panels
    ...
    print(
        ...
        f"  ARI={ari:.3f}   errors={n_err}/{len(dataset)}   "
        f"violations {viol_before}->{viol_after}   "
        f"changed={n_changed}   iters={n_iters}   "
